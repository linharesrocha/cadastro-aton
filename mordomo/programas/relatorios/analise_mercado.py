...
def netshoes(pesquisa):
    ...
    df['preco_de'] = df['preco_de'].str.replace(',','.')
    df['preco_por'] = df['preco_por'].str.replace(',','.')
    df['notas'] = df['notas'].str.replace(',','.')
    df['avaliacoes'] = df['avaliacoes']

    # Converte int para float
    try:
        df['preco_de'] = df['preco_de'].astype(float)
    except ValueError:
        print('Erro ao converter preco_de para float')
    try:
        df['preco_por'] = df['preco_por'].astype(float)
    except ValueError:
        print('Erro ao converter preco_por para float')
    try:
        df['notas'] = df['notas'].astype(float)
    except ValueError:
        print('Erro ao converter notas para float')
    try:
        df['avaliacoes'] = df['avaliacoes'].astype(int)
    except ValueError:
        print('Erro ao converter avaliacoes para float')
...
def converte_excel(df, pesquisa):
    ...
    if dados_ws.max_row > 1:
        ...
        tabela.sort(key=lambda x: str(x[1]), reverse=True)
...
if __name__ == '__main__':
    ...
    converte_excel(df=df, pesquisa=pesquisa)